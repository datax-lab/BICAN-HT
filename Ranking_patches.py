...
train_loader = create_dataloader(
    folder_path=f"/home/yaganapu/senescence/data/updated_model_data/whole_image/iterations_for_proposed_model/iteration_{iteration}/train",
    patch_extractor=patch_extractor,
    transform=transform,
    model=binary_model,
    device=device,
...
save_dir = f"./1000epoch_binary_classifier_embeddings_overlapping_patches_256dim_{batch_size}_{embedding_dim}_{temperature}_h{hierarchy_weight}/iteration_{iteration}"
...
for epoch in range(num_epochs):
    ...
    for batch in train_loader:
        ...
        labels = batch["true_label"].float().to(device).view(-1, 1)

        outputs, nucleus_probs, non_nucleus_probs = binary_model(
            nucleus_patches,
            non_nucleus_patches
        )
    ...
    print(
        ...
        f"Train AUC-PR: {train_auc_pr:.4f}"
    )
...
print(f"\nFinal binary classifier saved to:")
...
metrics_df = pd.DataFrame({
    ...
    "train_auc": train_auc_values,
    "val_auc": val_auc_values,
    "train_auc_pr": train_auc_pr_values,
    "val_auc_pr": val_auc_pr_values
})

metrics_csv_path = os.path.join(
    save_dir,
    "binary_classifier_metrics.csv"
)

metrics_df.to_csv(metrics_csv_path, index=False)

print(f"Metrics CSV saved to:")
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Binary Classifier Loss Curve")
...
loss_curve_path = os.path.join(
    save_dir,
    "binary_classifier_loss_curve.png"
)
